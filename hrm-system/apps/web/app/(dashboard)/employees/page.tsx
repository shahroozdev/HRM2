...
export default function EmployeesPage(): React.JSX.Element {
  ...
  const form = useForm<FormData>({ resolver: zodResolver(schema), defaultValues: { employmentType: "full_time", role: "employee" } as any });

  return (
    <div>
      <PageHeader title="Employees" description="Manage workforce records" action={canEdit(session?.user?.role, "employees") ? <button onClick={() => setOpen(true)} className="rounded-md bg-[var(--accent)] px-4 py-2 text-white" type="button">Add Employee</button> : null} />
